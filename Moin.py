...
# === Configuration ===
ADMIN_IDS = [6480787990]  # Replace with your admin user IDs
BOT_TOKEN = "7278781816:AAEuRI5I-zMcrdJ9-Tq09cLsqAiUzcmNrKo"  # Replace with your bot token
...
def process_key_redeem(message):
    ...
    if any(user['user_id'] == user_id for user in users):
        bot.send_message(message.chat.id, "✅𝙊𝙒𝙉𝙀𝙍- @VIP_DDoS_SELLER")
        return
...
def attack_command(message):
    if not any(user['user_id'] == message.from_user.id for user in users):
        bot.send_message(message.chat.id, "🔑 𝙉𝙊 𝘼𝙋𝙋𝙍𝙊𝙑𝘼𝙇 𝘽𝙀𝙔 𝙏𝙊 𝘿𝙈-> @VIP_DDoS_SELLER")
        return
